...
<head>
  ...
  <style>
    body { font-family: "Inter", "Segoe UI", Arial, sans-serif; background: #f5f7fb; color: #1f2937; margin: 0; }
    header { background: #0f172a; color: #fff; padding: 40px; }
    header h1 { margin: 0 0 8px 0; font-size: 32px; }
    header p { margin: 0; opacity: 0.8; }
    .container { padding: 32px; }
    .cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 16px; }
    .card { background: #fff; border-radius: 12px; padding: 16px; box-shadow: 0 4px 12px rgba(15, 23, 42, 0.08); }
    ...
    .finding { background: #fff; border-radius: 12px; padding: 16px; margin-bottom: 16px; box-shadow: 0 4px 12px rgba(15, 23, 42, 0.08); }
...
<body>
  ...
  <div class="container">
    ...
    <section class="section">
      ...
      <p>Host analisado: <strong>{{ system.hostname }}</strong> ({{ system.os_name }} {{ system.os_version or "" }})</p>
      <p>Kernel: {{ system.kernel }}</p>
      <p class="muted">Este relatório consolida evidências fornecidas e inventário local, correlacionando CVEs com base em fontes públicas.</p>
    </section>
    ...
    <section class="section">
      ...
      {% for finding in findings %}
        <div class="finding">
          ...
          <p class="muted">Versão detectada: {{ finding.version or "N/A" }} | Severidade: {{ finding.severity or "N/A" }}</p>
...
def write_html(report: FindingsReport, output_path: Path, redact: bool, client_name: str | None) -> None:
